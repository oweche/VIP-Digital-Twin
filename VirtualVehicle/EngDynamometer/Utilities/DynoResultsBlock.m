...
dynoapp = bdroot(Block);
DynoCtrlBlk = [dynoapp,'/Dynamometer Control'];
mode = get_param(DynoCtrlBlk, 'LabelModeActiveChoice');
% Close waitbar
if ~noplot
...
if ~noplot && simoutexist && ...
        strcmp(get_param(Block, 'DispAtEndCheckbox'), 'on') && ...
        ~strcmp(mode,'CalThrWg') && ~strcmp(mode,'CalCtrl')
    PlotDynamicButtonCallback(Block)
end
end
